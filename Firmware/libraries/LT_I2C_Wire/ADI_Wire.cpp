...
#include "ADI_Wire.h"  // must be included before Arduino.h
                       // to override SERCOM.h with our own ADI_SERCOM.h
...
void TwoWire::begin(void) {
  //Master Mode
  sercom->initMasterWIRE(TWI_CLOCK, DEFAULT_WIRE_TIMEOUT);
  sercom->enableWIRE();
...
void TwoWire::begin(uint32_t timeoutMicroSeconds) {
  //Master Mode with explicit timeout
    sercom->initMasterWIRE(TWI_CLOCK, timeoutMicroSeconds);
    sercom->enableWIRE();
