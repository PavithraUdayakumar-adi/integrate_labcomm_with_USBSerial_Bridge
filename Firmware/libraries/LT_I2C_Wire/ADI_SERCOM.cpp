/*
  ...
  Modifications to improve robustness to i2c bus electrical faults
  are copyright (c) 2018 Analog Devices. All rights reserved.
...
#include "ADI_SERCOM.h"
#include "variant.h"
#include "delay.h"       // Needed to use Arduino's micros() function to implement i2c timeouts.
#include "wiring_digital.h"    // Needed to use digitalRead(), digitalWrite(), and pinMode()
#include "wiring_constants.h"  // to reset slaves that hold SDA low in the Wire interface.
...
void SERCOM::initMasterWIRE(uint32_t baudrate, uint32_t timeoutMicroSeconds)
{
  masterI2Ctimeout = timeoutMicroSeconds;
...
//  sercom->I2CM.INTENSET.reg = SERCOM_I2CM_INTENSET_MB | SERCOM_I2CM_INTENSET_SB | SERCOM_I2CM_INTENSET_ERROR ;
  ...
  // Enable Inactivity Timeout and also the SCL low timeouts

  sercom->I2CM.CTRLA.reg |= SERCOM_I2CM_CTRLA_MEXTTOEN | SERCOM_I2CM_CTRLA_SEXTTOEN | SERCOM_I2CM_CTRLA_LOWTOUTEN
...
bool SERCOM::startTransmissionWIRE(uint8_t address, SercomWireReadWriteFlag flag)
{
  ...
  bool SDA_stuck_low = true;
  // Wait idle or owner bus mode
  while ( !isBusIdleWIRE() && !isBusOwnerWIRE() )
  {
    // Check if SDA is stuck low; the 4162 can do this if there's noise on SDA or SCL
    // that makes it think there's still an ongoing i2c transaction. The 4162's port
    // does NOT implement SMBus timeouts.
    SDA_stuck_low &= !digitalRead(SDA);
    // If a timeout is set, see if we've timed out, and give up if we have.
    if (masterI2Ctimeout != 0 && micros() - t_start > masterI2Ctimeout)
      goto timed_out;
  }
  ...
  if ( flag == WIRE_WRITE_FLAG ) // Write mode
  {
    while( !sercom->I2CM.INTFLAG.bit.MB )
    {
      ...
      // If a bus error occurs or we lose arbitration, the MB bit may never be set.
      // Check for these conditions and bail out.
      if (sercom->I2CM.STATUS.bit.ARBLOST || sercom->I2CM.STATUS.bit.BUSERR)
      {
        return false;
      }
      // If a timeout is specified, check for it.
      if (masterI2Ctimeout != 0 && micros() - t_start > masterI2Ctimeout) {
        goto timed_out;
      }
    }
  ...
  else  // Read mode
  {
    while( !sercom->I2CM.INTFLAG.bit.SB )
    {
      ...
      // Check for timeout if one is specified.
      if (masterI2Ctimeout != 0 && micros() - t_start > masterI2Ctimeout)
      {
        goto timed_out;
      }
  ...
  if(sercom->I2CM.STATUS.bit.RXNACK || sercom->I2CM.STATUS.bit.ARBLOST)
  {
    return false;
  }
  else
  ...
  timed_out:
    ...
    {
      t_start = micros();
      while (digitalRead(SDA) == 0 && (micros() - t_start < masterI2Ctimeout))
      {
        // Wiggle SCL until whoever is holding SDA low releases it
        // or a second timeout period elapses.
        digitalWrite(SCL, 0);
        pinMode(SCL, OUTPUT);    // Pull down strongly on SCL for 50us.
        delayMicroseconds(50U);
        pinMode(SCL, INPUT);     // Hi-Z SCL for 50us.
        delayMicroseconds(50U);
      }
      pinMode(SCL, INPUT);
    }
    sercom->I2CM.STATUS.bit.BUSSTATE = WIRE_IDLE_STATE;  // Force state machine back to IDLE.
    while (sercom->I2CM.SYNCBUSY.bit.SYSOP);  // Wait for synchronized write to STATUS.BUSSTATE.
    return false; 
}
...
bool SERCOM::sendDataMasterWIRE(uint8_t data)
{
  ...
  if(sercom->I2CM.STATUS.bit.RXNACK)
    ...
    return true;
  // Reset state machine to IDLE if we timed out.
  timed_out:
    sercom->I2CM.STATUS.bit.BUSSTATE = WIRE_IDLE_STATE;  // Force state machine back to IDLE.
    while (sercom->I2CM.SYNCBUSY.bit.SYSOP);  // Wait for synchronized write to STATUS.BUSSTATE.
    return false;
...
uint8_t SERCOM::readDataWIRE( void )
{
  if(isMasterWIRE())
  {
    ...
    while( sercom->I2CM.INTFLAG.bit.SB == 0 )
    {
      ...
      // ...unless a bus error occurs or we time out:
...
void SERCOM::initClockNVIC( void )
{
  ...
  GCLK->CLKCTRL.reg = GCLK_CLKCTRL_ID( GCM_SERCOMx_SLOW ) | // Generic Clock 0x13 (SMBus/i2c timeouts for all SERCOMs)
                      GCLK_CLKCTRL_GEN_GCLK1              | // Generic Clock Generator 1 (32kHz xtal osc)
                      GCLK_CLKCTRL_CLKEN;                   //   must be the source per datasheet 28.6.3
