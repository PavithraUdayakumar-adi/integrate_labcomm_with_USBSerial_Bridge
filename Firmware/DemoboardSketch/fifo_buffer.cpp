...
int16_t fifoBuf_getBytePeekAhead(t_fifo_buffer *buf, uint16_t position)
{	// get a data byte from the buffer without removing it

    uint16_t rd = buf->rd + position;
	uint16_t buf_size = buf->buf_size;
    ...
    if (num_bytes < 1)
        return -1;                      // no byte retuened
	if (position + 1 > num_bytes)		// byte requested does not exist
		return -1;
...
{       // get data from our rx buffer
    ...
    buf->rd = rd;
    SerialUSB.println("Exit fifo get data....");
    return i;                   // return number of bytes copied
}
